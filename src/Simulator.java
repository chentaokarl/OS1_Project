...
/**
 * simulator based on discrete time in terms of VTUs run three simulations in
 * once but separately
 * 
 * @author Chen
 *
	...
	public void simulate(String strategy, BufferedWriter output) throws IOException {
		...
		while (timer < SIMULATION_TIME) {
			...
			if ((jobInProcessing.getProcessedTime() + jobInterval) >= jobInProcessing.getDuration()) {
				...
				jobInProcessing
						.setEndTime(timer + (jobInProcessing.getDuration() - jobInProcessing.getProcessedTime()));
				...
				if (jobInProcessing.getStartTime() >= 1000 && jobInProcessing.getEndTime() <= 4000) {
					...
					if ((jobInProcessing.getEndTime() - jobInProcessing.getStartTime()) < jobInProcessing
							.getDuration()) {
