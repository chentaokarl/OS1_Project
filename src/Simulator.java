...
public class Simulator {
	...
	public static void main(String[] args) {
		...
		try {
			// I print infomation both on stdout(screen) and into a file
			out = new BufferedWriter(
					new FileWriter("WorstFit_Result_" + Calendar.getInstance().getTimeInMillis() + ".txt"));
	...
	public void simulate(String strategy, BufferedWriter output) throws IOException {
		...
		while (timer < SIMULATION_TIME) {
			...
			if (null == jobInProcessing) {
				if (jobsList.isEmpty()) {
					newArriveJob(jobInterval, strategy);
					continue;
				}
			...
			if ((jobInProcessing.getProcessedTime() + jobInterval) >= jobInProcessing.getDuration()) {
				...
				if ((jobInProcessing.getProcessedTime() + jobInterval) - jobInProcessing.getDuration() > 0) {
					...
					// next job start at the end of current job, temp = time needed to finish current job
					temp = jobInProcessing.getDuration() - jobInProcessing.getProcessedTime();
					// run the next job in the list which
					...
					if ((jobInterval - temp) < jobInProcessing.getDuration()) {
						jobInProcessing.setProcessedTime(jobInterval - temp);
					} else if ((jobInterval - temp) >= jobInProcessing.getDuration()) {
						finishJob();
						if ((timer + jobInterval - temp) == jobInProcessing.getDuration()) {
							...
							//time slice left over
							temp = jobInterval - temp - jobInProcessing.getDuration();
							jobInProcessing = jobsList.remove(0);
							jobInProcessing.setStatus(JobStatus.PROCESSING);
							jobInProcessing
									.setProcessedTime(temp);
						}
					}
				} else {
					jobInProcessing = null;
			...
			if (temp >= 1000 && temp <= 4000) {
				if (temp / 100 > timer / 100) {
					...
					System.out.print(", Storage Utilization: "
							+ Math.floor(memManager.storageUtilization() * 10000) / 100 + "%");
			...
			if (timer < 4000 && temp >= 4000) {
				...
				output.newLine();
				output.write("Average turnaround time(VTUs):"
						+ (Math.floor(turnaroundTimeCounter * 100.0 / processedJobsCounter) / 100));
				output.write(", Average waiting time(VTUs):"
						+ (Math.floor((turnaroundTimeCounter - processingTimeCounter) * 100.0 / processedJobsCounter)
								/ 100));
				output.write(", Average processing time(VTUs):"
						+ (Math.floor(processingTimeCounter * 100.0 / processedJobsCounter) / 100));
				output.newLine();
			}
	...
	private void newArriveJob(int jobInterval, String strategy) {
		timer += jobInterval;
		if (null == jobAtDoor) {
